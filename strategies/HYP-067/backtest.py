"""
HYP-067: Per-position stop-loss -30% på HYP-066:s återköpsintensitet-decil.

Se research/hypothesis_registry/HYP-067-stoploss-hyp066.yaml för det
lasta kriteriet. Forsta kandidaten i BATCH-004 (2 hypoteser, se
registerpostens header for diagnostikmotiveringen: HYP-066:s varsta
drawdown var INTE SPY-krasch-driven, darfor en direkt per-position-
atgard istallet for en marknadsbred trigger).

IDENTISK motor/signal som HYP-066 (aterananvands rakt av, se
strategies/HYP-066/backtest.py for full dokumentation av sjalva
signalen/motorn). ENDA TILLAGGET: ett DAGLIGT per-position stop-loss-
lager, EFTER SPY-beta-hedgen i samma loop-iteration, IDENTISK konvention
som HYP-025 (BATCH-001): -30%-troskel, en enda dags observation racker,
stang hela positionen, ingen automatisk aterkop fore nasta ombalansering.
"""
...
STOP_LOSS_THRESHOLD = -0.30  # IDENTISK konvention som HYP-025 (BATCH-001)
...
def run_backtest(close, close_adj, hedge, buyback_data, beta_df, spread_df, volume,
    ...
    for date_i in range(start_idx, len(close.index)):
        ...
        # ── STOP-LOSS-KONTROLL (NY i HYP-067, EFTER ombalanseringen i samma
        # loop-iteration, VARJE handelsdag - identisk konvention som HYP-025) ──
        for t in list(holdings.keys()):
            ti = tidx[t]
            cp = prices_v[date_i, ti]
            if np.isnan(cp):
                continue
            unrealized_ret = cp / holdings[t]["entry"] - 1
            if unrealized_ret < STOP_LOSS_THRESHOLD:
                proceeds = holdings[t]["shares"] * cp
                exit_spread = spread_v[date_i, ti]
                if not np.isnan(exit_spread):
                    proceeds -= proceeds * (exit_spread / 2)
                ...
                ret = proceeds / holdings[t]["cost"] - 1
                trade_log.append({"date": date, "ticker": t, "ret": ret,
                                   "gross_ret": unrealized_ret, "type": "stop_loss_exit",
                                   "cost": holdings[t]["cost"]})
                stop_loss_log.append({"date": date, "ticker": t, "ret": unrealized_ret})
                del holdings[t]
...
# HYP-066:s egna resultat (lasta i registret, jamforelsepunkten for villkor 2)
HYP066_MAIN_MAXDD = {100_000: -0.6685, 1_000_000: -0.7090, 10_000_000: -0.7264}
...
def main():
    ...
    for level in [100_000, 1_000_000, 10_000_000]:
        ...
        pv, tl, sl = run_backtest(close, close_adj, hedge, buyback_data, beta_df, spread_df, volume,
        ...
        sl.to_csv(RESULTS_DIR / f"stop_loss_log_{int(level)}.csv", index=False)

        ref_maxdd = HYP066_MAIN_MAXDD[level]
        cond2_level = m["max_drawdown"] > ref_maxdd
        print(f"    HUVUDPERIOD: CAGR={m['cagr']:+.2%}  Sharpe={m['sharpe']:.4f}  "
              f"Calmar={m['calmar']:.3f}  MaxDD={m['max_drawdown']:.2%} "
              f"(HYP-066 ref: {ref_maxdd:.2%}, {'BÄTTRE' if cond2_level else 'EJ BÄTTRE'})  "
              f"Trades={m['trade_stats']['n']}  Stop-loss-exits={m['n_stop_loss_exits']}")
    ...
    for m in main_results:
        lvl = m["capital_level"]
        c1 = m["sharpe"] >= 0.55
        c2 = m["max_drawdown"] > HYP066_MAIN_MAXDD[lvl]
        conds.append((lvl, c1, c2))

    overall = "PASSED" if all(c1 and c2 for _, c1, c2 in conds) else "FAILED"

    print("=== SLUTBEDOMNING (bada villkoren maste halla PA ALLA TRE NIVAER) ===")
    for lvl, c1, c2 in conds:
        print(f"  ${lvl:>10,.0f}  Villkor 1 (Sharpe>=0,55): {'PASS' if c1 else 'FAIL'}   "
              f"Villkor 2 (MaxDD battre an HYP-066): {'PASS' if c2 else 'FAIL'}")
    print(f"  => {overall}\n")

    summary = {"main": main_results, "oos_2025": oos_results, "hyp066_main_maxdd_ref": HYP066_MAIN_MAXDD,
               "pass_fail": {"per_level": [{"level": l, "condition_1_sharpe": bool(c1), "condition_2_maxdd": bool(c2)}
                                            for l, c1, c2 in conds],
                             "overall": overall}}
