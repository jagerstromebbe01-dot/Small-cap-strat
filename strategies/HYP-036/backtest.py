"""
HYP-036: Sektorneutral idiosynkratisk-volatilitets-rankning pa HYP-023.

Se research/hypothesis_registry/HYP-036-sektorneutral-idio-vol-hyp023.yaml
for det lasta kriteriet.

MOTIV: scripts/attribution.py (byggt 2026-08-01) visade att HYP-023 har
5.6x overexponering mot statliga affarsbanker (SIC 60) mot
basuniversumet, och HYP-008 hade 9.0x - ett upprepat, uppmatt monster,
inte en teoretisk oro.
...
OFORANDRAT): rankningssteget byts fran GLOBALT till SEKTORNEUTRALT.
Kandidater grupperas efter SIC-huvudgrupp (strategies/common/sector.py::
load_ticker_major_group(), samma funktion som attribution.py anvander),
och DECILE_FRACTION=0.10 tas INOM varje sektor separat istallet for en
gang globalt over hela eligible-listan. Slutlig malportfolj = unionen av
alla sektorers delval. Sjalva idio-vol-BERAKNINGEN ar oforandrad -
mekanismen paverkar bara rankningens SCOPE.
"""
...
from sector import load_bank_financial_flags, load_ticker_major_group  # noqa: E402
...
def run_backtest(close, close_adj, hedge, vol_df, beta_df, spread_df, volume,
                 universe_by_month, capital_level: float, bank_flags: dict, major_group_map: dict):
    """
    ...
    for date_i in range(start_idx, len(close.index)):
        ...
        if date in rebal_set:
            ...
            # SEKTORNEUTRAL RANKNING (HYP-036): gruppera per SIC-huvudgrupp,
            # ta DECILE_FRACTION INOM varje sektor separat (samma formel som
            # den tidigare globala rankningen, bara ateranvand per sektor),
            # union av alla sektorers delval. Okand SIC ("XX") ar sin egen
            # hink - exkluderas inte.
            sector_candidates = {}
            for t in eligible:
                v = vol_today[t]
                cp = prices_v[date_i, tidx[t]]
                if not np.isnan(v) and not np.isnan(cp) and cp > 0:
                    mg = major_group_map.get(t, "XX")
                    sector_candidates.setdefault(mg, []).append((v, t, cp))
            target_tickers = set()
            for mg, scores in sector_candidates.items():
                scores.sort()  # LAGST idiosynkratisk vol forst, INOM sektorn
                n_top = max(1, int(len(scores) * DECILE_FRACTION))
                target_tickers.update(t for _, t, _ in scores[:n_top])
...
def run_for_capital_level(capital_level, close, close_adj, hedge, vol_df, beta_df, spread_df, volume,
                           universe_by_month, bank_flags, major_group_map):
    print(f"    Kör backtest (kapitalnivå {capital_level:,.0f})...")
    pv, tl, stl = run_backtest(close, close_adj, hedge, vol_df, beta_df, spread_df, volume, universe_by_month,
                                capital_level, bank_flags, major_group_map)
...
def main():
    ...
    print("Laddar SIC-huvudgrupper for sektorneutral rankning (data/cache/sic_classification.jsonl)...")
    major_group_map = load_ticker_major_group(tickers)
    n_known = sum(1 for mg in major_group_map.values() if mg != "XX")
    print(f"  {n_known} av {len(tickers)} tickers har kand SIC-huvudgrupp.\n")
