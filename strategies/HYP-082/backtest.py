"""
HYP-082: Naiv 50/50-kombination (kvartalsvis ombalanserad) - HYP-079 +
Leg 2 (HYP-037, idio-vol+krasch). Se
research/hypothesis_registry/HYP-082-naiv-kombination-hyp079-hyp037.yaml
for det lasta kriteriet. Samma metod som HYP-039/078/081.
"""
...
HYP037_SHARPE = {100_000: 0.7366, 1_000_000: 0.7547, 10_000_000: 0.6981}
HYP037_MAXDD = {100_000: -0.2309, 1_000_000: -0.1911, 10_000_000: -0.1310}
...
def main():
    ...
    print("=== HYP-082: naiv 50/50 HYP-079 + Leg 2 (HYP-037) ===\n")
    main_results = []

    for level in levels:
        pv037 = pd.read_csv(HYP037_RESULTS / f"portfolio_value_{level}.csv",
                             index_col=0, parse_dates=True)["portfolio_value"]
        pv079 = pd.read_csv(HYP079_RESULTS / f"portfolio_value_{level}.csv",
                             index_col=0, parse_dates=True)["portfolio_value"]
        combined = combine_50_50(pv037, pv079)
        ...
        best_sharpe = max(HYP037_SHARPE[level], HYP079_SHARPE[level])
        best_maxdd = max(HYP037_MAXDD[level], HYP079_MAXDD[level])
    ...
    cond1_all = all(main_results[i]["sharpe"] >= max(HYP037_SHARPE[levels[i]], HYP079_SHARPE[levels[i]]) for i in range(3))
    cond2_all = all(main_results[i]["max_drawdown"] > max(HYP037_MAXDD[levels[i]], HYP079_MAXDD[levels[i]]) for i in range(3))
