...
HYP-029 (BATCH-001, kandidat 5/5): TREDJE lager, ett per-position
stop-loss pa -15% (GRUNDARE an HYP-018:s -20%, motsatt riktning mot
batchens ovriga kandidater), EFTER bade SPY-overlayen och sektor-
triggern. Se
research/hypothesis_registry/HYP-029-stoploss-15pct.yaml for det lasta
...
# Per-position stop-loss (ny i HYP-029, BATCH-001 kandidat 5/5 - se
...
def run_backtest(close, close_adj, hedge, vol_df, beta_df, spread_df, volume,
    ...
    for date_i in range(start_idx, len(close.index)):
        ...
        # ── Per-position stop-loss (ny, HYP-029/BATCH-001): DAGLIG
        # kontroll, EFTER bade SPY-overlayen och sektor-triggern.
        for t in list(holdings.keys()):
            ti = tidx[t]
            cp = prices_v[date_i, ti]
            if np.isnan(cp):
