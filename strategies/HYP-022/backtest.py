"""
HYP-022: Branschtak (bank/finans) pa HYP-017 (idiosynkratisk vol + SPY-
krasch-overlay, PASSED, K=16, fortfarande referensimplementationen).

Se research/hypothesis_registry/HYP-022-branschtak-hyp017.yaml for det
lasta kriteriet, och research/strategy_specs/HYP-022-spec.md for den
fulla specen.

DIAGNOS (skeptisk granskning 2026-07-30, INNAN denna hypotes lastes):
HYP-017:s 696 nagonsin handlade tickers ar 27.3% bank-/finansrelaterade
(bancorp, savings, bank, thrift, financial, trust i bolagsnamnet) mot
7.0% i hela small-cap-basuniversumet - en KAND mekanisk sidoeffekt av
lagvolatilitets-/idio-vol-siktning. Konkret skadefall: amerikanska
regionbankskrisen mars 2023 gav portfoljen -14.8% (2023-02-01 till
2023-05-01, $100k-nivan) medan SPY var +1.5% samma period - krasch-
overlayen (bevakar bara SPY) triggade aldrig, eftersom kraschen var
sektorspecifik.

TILLAGG mot HYP-017 (allt annat - signal, universum, friktion, hedge,
kapacitetsspärr, krasch-overlay, de tre motorfixarna - AR OFORANDRAT):
  Ett TAK pa bank-/finanssektorns andel av de valda namnen vid varje
  kvartalsvis decil-ombalansering: max floor(n_top * 0.15) namn (2x
  basuniversumets 7.0%-representation, avrundat) far vara bank-/
  finansklassificerade. En bankkandidat som skulle bryta mot taket
  hoppas over och ersatts langre ner i decil-kon av nasta kandidat.
"""
...
# Branschtak (ny i HYP-022, se pass_fail_criterion - lockat, ej fri parameter):
# 2x basuniversumets bank-/finansandel (7.0%), avrundat till 15%.
BANK_CAP_FRACTION = 0.15
...
def run_backtest(close, close_adj, hedge, vol_df, beta_df, spread_df, volume,
    ...
    for date_i in range(start_idx, len(close.index)):
        ...
        if date in rebal_set:
            ...
            # BRANSCHTAK (ny i HYP-022): ga igenom kandidaterna i ordning
            # (lagst idio-vol forst), hoppa over en bankkandidat om
            # bankkvoten redan ar fylld - den ersatts av nasta kandidat
            # langre ner i kon (bank eller icke-bank). Icke-bank-
            # kandidater paverkas inte av taket.
            target_tickers = set()
            bank_selected = 0
            ...
            sector_check_log.append({
                "date": date, "n_selected": len(target_tickers),
                "n_bank": bank_selected,
                "bank_fraction": bank_selected / len(target_tickers) if target_tickers else 0.0,
            })
    ...
    sc = pd.DataFrame(sector_check_log)
    return pv, tl, sc
...
def march_2023_episode_return(pv):
    """Villkor 4 i HYP-022:s pass_fail_criterion: avkastning 2023-02-01
...
def run_for_capital_level(capital_level, close, close_adj, hedge, vol_df, beta_df, spread_df, volume,
    ...
    pv, tl, sc = run_backtest(close, close_adj, hedge, vol_df, beta_df, spread_df, volume, universe_by_month,
                               capital_level, bank_flags)

    max_bank_fraction = float(sc["bank_fraction"].max()) if len(sc) else 0.0
    ...
    result = {
        ...
        "max_bank_fraction_any_rebalance": max_bank_fraction,
    ...
    sc.to_csv(RESULTS_DIR / f"sector_check_{int(capital_level)}.csv", index=False)
...
def main():
    ...
    for level in levels:
        ...
        print(f"    CAGR={result['cagr']:+.2%}  Sharpe={result['sharpe']:.2f}  "
              ...
              f"MaxBankFraction={result['max_bank_fraction_any_rebalance']:.1%}  "
