"""
HYP-081: Naiv 80/20-kombination (kvartalsvis ombalanserad) - HYP-056
(befintlig referensportfölj) + HYP-079 (femte ben, branschtaksvarianten).

Se research/hypothesis_registry/HYP-081-naiv-femte-ben-hyp079-hyp056.yaml
for det lasta kriteriet. Identisk metod som HYP-078 - HYP-075 utbytt mot
dess starkare efterföljare HYP-079.
"""
...
HYP078_RESULTS = STRATEGIES_ROOT / "HYP-078" / "results"
...
HYP078_SHARPE = {100_000: 1.3756, 1_000_000: 1.3469, 10_000_000: 1.3056}
HYP078_MAXDD = {100_000: -0.0420, 1_000_000: -0.0426, 10_000_000: -0.0398}

OOS_START = "2025-01-01"


def combine_80_20(pv056: pd.Series, pv079: pd.Series, start_capital: float = 1.0) -> pd.Series:
    df = pd.concat([pv056.rename("hyp056"), pv079.rename("hyp079")], axis=1, join="inner").dropna()
    r056 = df["hyp056"].pct_change()
    r079 = df["hyp079"].pct_change()
    ...
    leg079 = start_capital * WEIGHT_HYP079
    values, dates = [], []

    for i in range(1, len(df)):
        date = df.index[i]
        r1, r2 = r056.iloc[i], r079.iloc[i]
        if not np.isnan(r1):
            leg056 *= (1 + r1)
        if not np.isnan(r2):
            leg079 *= (1 + r2)
        ...
        if date in rebal_set:
            ...
            target079 = total * WEIGHT_HYP079
            turnover = abs(target056 - leg056) + abs(target079 - leg079)
            total -= turnover * REBALANCE_COST_BPS
            leg056 = total * WEIGHT_HYP056
            leg079 = total * WEIGHT_HYP079
        values.append(total)
        dates.append(date)
...
def main():
    ...
    print("=== HYP-081: naiv 80/20 HYP-056 + HYP-079 ===\n")
    ...
    for level in levels:
        ...
        pv079 = pd.read_csv(HYP079_RESULTS / f"portfolio_value_{level}.csv",
                             index_col=0, parse_dates=True)["portfolio_value"]
        main_pv = combine_80_20(pv056, pv079.loc[pv079.index < OOS_START])
        ...
        pv056_oos = pd.read_csv(HYP056_RESULTS / f"portfolio_value_oos2025_combined_scaled_{level}.csv",
                                 index_col=0, parse_dates=True)["portfolio_value"]
        pv079_oos = pv079.loc[pv079.index >= OOS_START]
        oos_pv = combine_80_20(pv056_oos, pv079_oos)
        ...
        print(f"  ${level:>10,.0f}  Sharpe={m['sharpe']:.4f} (HYP-056:{HYP056_SHARPE[level]:.4f}, "
              f"HYP-078:{HYP078_SHARPE[level]:.4f}) -> {'PASS' if cond1 else 'FAIL'}")
        print(f"               MaxDD={m['max_drawdown']:.2%} (HYP-056:{HYP056_MAXDD[level]:.2%}, "
              f"HYP-078:{HYP078_MAXDD[level]:.2%}) -> {'PASS' if cond2 else 'FAIL'}")
