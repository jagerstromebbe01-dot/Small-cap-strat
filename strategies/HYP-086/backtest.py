"""
HYP-086: 8-K-katalysatortiming som extra kortbenskrav pa HYP-079. Se
research/hypothesis_registry/HYP-086-8k-katalysator-timing-hyp079.yaml
for det lasta kriteriet.

Identisk motor/signal/branschtak/positionstak som HYP-079. NY REGEL:
short-benet kraver DESSUTOM minst en 8-K inom trailing 30 kalenderdagar
med item 1.01, 3.02 eller 8.01. Long-benet ofOrandrat.
"""
...
from sec_filing_index import load_filing_index, S3_FAMILY_FORMS, has_item  # noqa: E402
...
# ════════════════════════════════════════════════════════════
#  SHELF-SIGNAL + 8-K-KATALYSATOR (ny i HYP-086)
# ════════════════════════════════════════════════════════════
...
def has_recent_catalyst(filings, as_of_ts):
    for f in filings:
        if f.get("form") != "8-K":
            continue
        fdate = f.get("filingDate")
        ...
        if fts >= as_of_ts:
            continue
        if (as_of_ts - fts).days > CATALYST_WINDOW_DAYS:
            continue
        if any(has_item(f, code) for code in CATALYST_ITEMS):
            return True
    return False
...
def classify_ticker(filing_index, ticker, as_of_ts):
    ...
    if c12 >= ACTIVE_12M_MIN_COUNT and c6 >= ACTIVE_6M_MIN_COUNT:
        if has_recent_catalyst(filings, as_of_ts):
            return "active"
        return None  # uppfyller shelf-kravet men saknar 8-K-katalysator - varken/eller
    if c24 == 0:
        return "clean"
    return None
...
# ════════════════════════════════════════════════════════════
#  MOTOR (identisk med HYP-079, andrad klassificering ovan)
# ════════════════════════════════════════════════════════════
def run_backtest(close, close_adj, spread_df, volume, filing_index, universe_by_month, sic_map,
    ...
    for date_i in range(start_idx, len(close.index)):
        ...
        if date in rebal_set:
            ...
            short_count_log.append({"date": str(date.date()), "n_short": len(target_short)})
    ...
    avg_short_names = float(np.mean([x["n_short"] for x in short_count_log])) if short_count_log else 0.0
    return pv, tl, trl, avg_short_names
...
def main():
    ...
    kurt_100k, sharpe_ex2020_100k, avg_short_100k = None, None, None

    for level in [100_000, 1_000_000, 10_000_000]:
        print(f"--- Kapitalnivå: ${level:,.0f} ---")
        pv, tl, trl, avg_short_names = run_backtest(close, close_adj, spread_df, volume, filing_index,
        ...
        m = {"capital_level": level, "cagr": cagr(main_pv), "sharpe": sharpe(main_pv),
             "max_drawdown": max_drawdown(main_pv), "n_trims": len(trl), "avg_short_names": avg_short_names}
        ...
        m["kurtosis"] = kurt
        if level == 100_000:
            kurt_100k, sharpe_ex2020_100k, avg_short_100k = kurt, sharpe_ex2020, avg_short_names
        ...
        print(f"    Sharpe(ex-2020)={sharpe_ex2020:.4f}  Kurtosis={kurt:.1f}  Genomsnittligt antal short-namn/manad={avg_short_names:.1f}")
        print(f"    OOS-2025: Sharpe={o['oos_2025_sharpe']}  MaxDD={o['oos_2025_max_drawdown']}\n")

    m100k = main_results[0]
    feasibility_fail = avg_short_100k < MIN_AVG_SHORT_NAMES
    ...
    print(f"  Genomsnittligt antal short-namn/manad: {avg_short_100k:.1f} (min-krav: {MIN_AVG_SHORT_NAMES}) "
          f"-> {'FEASIBILITY-FAIL' if feasibility_fail else 'OK'}")
    if not feasibility_fail:
    ...
    summary = {"main": main_results, "oos_2025": oos_results, "feasibility_fail": bool(feasibility_fail),
