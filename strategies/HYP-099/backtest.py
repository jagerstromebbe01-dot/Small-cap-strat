"""
HYP-099 (BATCH-004, 3/4 accrual): skalning mot marknadsvärde, long-only
lägsta decilen. Se
research/hypothesis_registry/HYP-099-accrual-marknadsvarde-longonly.yaml
för det låsta kriteriet. Motorn (strategies/common/accrual_engine.py)
är DELAD med HYP-097/098/100 - kopiera INTE.
"""
import json
import sys
...
def main():
    print(f"=== HYP-099: Accrual (skalning={SCALING}, konstruktion={CONSTRUCTION}) ===\n")
    ...
    print("Laddar aktieantal (för marknadsvärde-skalning)...")
    shares_data = ae.load_shares_outstanding()
    shares_lookup = ae.make_shares_lookup(shares_data)
    print(f"  {len(shares_data)} tickers har aktieantal.\n")
    ...
    for level in levels:
        ...
        baseline = ae.equal_weighted_baseline(close, universe_by_month, 130)
        ...
        print(f"  Baseline (ofiltrerat, friktionsfri disclosure): Sharpe={sh_base:.4f}  MaxDD={md_base:.2%}\n")

        results.append({"capital_level": level, "sharpe": sh, "max_drawdown": md, "cagr": cg, "calmar": cm,
                         "dsr": dsr, "n_trades": len(tl),
