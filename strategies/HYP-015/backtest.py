"""
HYP-015: Idiosynkratisk volatilitet pa small-cap-universumet (uppfoljare
till HYP-014, total-volatilitetsfaktorn, som var FAILED men basta
resultatet av K=13 hypoteser sa langt).

Se research/hypothesis_registry/HYP-015-idiosynkratisk-volatilitet.yaml
for det lasta kriteriet, och research/strategy_specs/HYP-015-spec.md for
den fulla specen.

Aterananvander HYP-014:s motor rakt av (universum, friktion, SPY-hedge,
kapacitetsspärr, kvartalsvis ombalansering) - enda metodikskillnaden ar
signalen: IDIOSYNKRATISK volatilitet (std av regressionsresidualen efter
att marknadsbetan dragits bort) i stallet for TOTAL realiserad volatilitet
(Ang, Hodrick, Xing & Zhang 2006).

DESSUTOM (lockat som en del av kriteriet, delat med HYP-016, se
granskningen 2026-07-29 som hittade dessa i HYP-012/014):
  1. Ombalanseringsdatum snappas till narmaste faktiska handelsdag
     (strategies/common/rebalancing.py) - HYP-014:s kod tystade ~32% av
     kvartalsvisa ombalanseringar genom att matcha kalender-periodslut
     direkt mot handelskalendern (se ESSA-fallet i HYP-014:s registerpost).
  2. adjusted_close anvands for ALLA avkastnings-/signalberakningar
     (stock_ret, hedge_ret, beta, residual-vol) - INTE ra close, som
     tidigare gav falska volatilitetsspikar vid ojusterade aktiesplitar.
     Ra close anvands fortfarande for positionsvarde/exekvering.
  3. flag_implausible_liquidity() maskar ticker-dagar dar leverantors-
     ...
     clean_price_matrix() ensam fangar INTE detta monster.
"""
