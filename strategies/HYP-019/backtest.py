"""
HYP-019: Storlekssortering inom small-cap-bandet ($100M-$2B).

Se research/hypothesis_registry/HYP-019-storlekssortering-inom-bandet.yaml
for det lasta kriteriet, och research/strategy_specs/HYP-019-spec.md for
den fulla specen.

Aterananvander HYP-015:s motor rakt av (universum, friktion, SPY-hedge,
kapacitetsspärr, kvartalsvis ombalansering, de tre motorfixarna) - enda
metodikskillnaden ar signalen: MARKNADSVARDE i stallet for idiosynkratisk
volatilitet. Detta ar ETT RENT SIGNALTEST - ingen krasch-overlay eller
stop-loss (till skillnad fran HYP-017/018) - i linje med principen att
utvardera en kandidatsignals egen edge INNAN nagon riskoverlay laggs pa
(se konversationen 2026-07-30).

Signal: manadsvis marknadsvarde per ticker, fran redan befintlig
data/cache/market_cap_by_ticker_month.csv (samma kalla som redan bygger
universumklassificeringen - ingen ny datainhamtning). Ga LANG i LAGSTA
marknadsvardes-decilen INOM det redan small-cap-klassificerade bandet -
den mest direkta testen av projektets EGEN kapacitetsbegransningstes
(Berk & Green 2004), till skillnad fran HYP-012/013/014/015 som alla
importerade en redan kand stor-cap-faktor.

Motorfixar (delade med HYP-015/016/017/018): snappade ombalanseringsdatum,
adjusted_close genomgaende, flag_implausible_liquidity().
"""
...
MIN_HISTORY = 130   # samma buffert som HYP-015 - styrs av BETA_WINDOW (126), inte av signalen
                    # (marknadsvarde behover ingen rullande prishistorik, till skillnad fran vol/momentum)

DECILE_FRACTION = 0.10    # lagsta marknadsvardes-decilen
REBAL_FREQ = "QE"         # kvartalsvis - storlek ar en langsamt roterande karakteristika, samma konvention som HYP-014/015
RF_ANNUAL = 0.02
BETA_WINDOW = 126
...
# ════════════════════════════════════════════════════════════
#  SIGNAL: MARKNADSVARDE (ny - ersatter idiosynkratisk-vol-signalen)
# ════════════════════════════════════════════════════════════
def load_market_cap():
    """
    Manadsvis marknadsvarde per ticker, redan berakat och cachat (samma
    kalla som bygger smallcap_universe_by_month.json - ingen ny
    datainhamtning). Pivoterad till en DataFrame (index=manadsslutdatum,
    columns=ticker, values=market_cap) sa den kan slas upp direkt via
    samma kalenderetikett (rebal_map[date]) som redan anvands for
    universe_by_month-uppslagningen.
    """
    df = pd.read_csv(MARKET_CAP_FILE, parse_dates=["month"])
    return df.pivot(index="month", columns="ticker", values="market_cap")
...
# ════════════════════════════════════════════════════════════
def run_backtest(close, close_adj, hedge, mcap_df, beta_df, spread_df, volume,
    ...
    for date_i in range(start_idx, len(close.index)):
        ...
        if date in rebal_set:
            ...
            mcap_today = mcap_df.loc[rebal_map[date]] if rebal_map[date] in mcap_df.index else pd.Series(dtype=float)
            scores = []
            for t in eligible:
                v = mcap_today.get(t, np.nan)
                cp = prices_v[date_i, tidx[t]]
                if not np.isnan(v) and not np.isnan(cp) and cp > 0:
                    scores.append((v, t, cp))
            scores.sort()  # LAGST marknadsvarde forst
...
#  KÖRNING
# ════════════════════════════════════════════════════════════
def run_for_capital_level(capital_level, close, close_adj, hedge, mcap_df, beta_df, spread_df, volume, universe_by_month):
    print(f"    Kör backtest (kapitalnivå {capital_level:,.0f})...")
    pv, tl = run_backtest(close, close_adj, hedge, mcap_df, beta_df, spread_df, volume, universe_by_month, capital_level)
...
def main():
    ...
    print("Laddar marknadsvarde-signal (redan cachad, per manad)...")
    mcap_df = load_market_cap()
    ...
    for level in levels:
        ...
        result = run_for_capital_level(level, close, close_adj, hedge, mcap_df, beta_df, spread_df, volume, universe_by_month)
