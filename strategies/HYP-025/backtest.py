...
HYP-025 (BATCH-001, kandidat 1/5): TREDJE lager, ett per-position
stop-loss pa -30%, EFTER bade SPY-overlayen och sektor-triggern. Se
research/hypothesis_registry/HYP-025-stoploss-30pct.yaml for det lasta
kriteriet. Malfall: HYP-018 (samma mekanism pa HYP-017, -20%-troskel,
FAILED - for manga falska larm fran normala tillfalliga nedgangar).
"""
...
# Per-position stop-loss (ny i HYP-025, BATCH-001 kandidat 1/5 - se
...
def run_backtest(close, close_adj, hedge, vol_df, beta_df, spread_df, volume,
    ...
    for date_i in range(start_idx, len(close.index)):
        ...
        # ── Per-position stop-loss (ny, HYP-025/BATCH-001): DAGLIG
        # kontroll, EFTER bade SPY-overlayen och sektor-triggern (pa vad
        # som aterstar av varje position efter eventuella portfoljbreda/
        # sektorbreda nedskarningar samma dag). Stanger HELA positionen -
        # riktat mot EN position i taget, inte portfoljbrett. Kraver
        # STOP_LOSS_CONFIRM_DAYS pafoljande dagar under trosklen (=1 for
        # de flesta BATCH-001-kandidaterna, =3 for HYP-028).
        for t in list(holdings.keys()):
            ti = tidx[t]
            cp = prices_v[date_i, ti]
            if np.isnan(cp):
