"""
HYP-084: Pairwise korrelationsbudget (crowding-viktning) pa HYP-079. Se
research/hypothesis_registry/HYP-084-korrelationsbudget-hyp079.yaml for
det lasta kriteriet.

Identisk motor/signal/branschtak/positionstak som HYP-079. NY REGEL:
efter branschtaksviktning, multiplicera varje ny entrys vikt med
(1 - crowding_i), dar crowding_i ar medelvardet av dess POSITIVA
60-dagars pairwise-korrelationer mot ovriga nya entries pa samma ben.
"""
...
HYP079_SHARPE_100K = 0.8433
HYP079_MAXDD_100K = -0.0582
HYP079_SHARPE_EX2020_100K = 0.8043
KURTOSIS_TOLERANCE = 10.56
...
# ════════════════════════════════════════════════════════════
#  SHELF-SIGNAL (identisk med HYP-072/075/079)
# ════════════════════════════════════════════════════════════
...
# ════════════════════════════════════════════════════════════
#  SEKTORTAKS-VIKTNING (identisk med HYP-079)
# ════════════════════════════════════════════════════════════
...
# ════════════════════════════════════════════════════════════
#  KORRELATIONSBUDGET (ny i HYP-084)
# ════════════════════════════════════════════════════════════
def crowding_scores(candidates, close_adj_values, tidx, date_i, window=CORR_WINDOW,
                    min_periods=CORR_MIN_PERIODS):
    candidates = list(candidates)
    if len(candidates) < 2:
        return {t: 0.0 for t in candidates}
    start = max(0, date_i - window)
    cols = [tidx[t] for t in candidates]
    window_prices = close_adj_values[start:date_i, :][:, cols]
    with np.errstate(invalid="ignore", divide="ignore"):
        rets = window_prices[1:] / window_prices[:-1] - 1.0
    df = pd.DataFrame(rets, columns=candidates)
    ...
    for t in candidates:
        ...
        pos = row.clip(lower=0).fillna(0.0)
        scores[t] = float(pos.mean()) if len(pos) > 0 else 0.0
    return scores


# ════════════════════════════════════════════════════════════
#  MOTOR (HYP-079 + korrelationsbudget vid sizing av nya entries)
# ════════════════════════════════════════════════════════════
def run_backtest(close, close_adj, spread_df, volume, filing_index, universe_by_month, sic_map,
    ...
    close_adj_values = close_adj.values
    ...
    for date_i in range(start_idx, len(close.index)):
        ...
        if date in rebal_set:
            ...
            avg_crowding_long = None
            if new_long:
                sector_w = {t: long_weights_full.get(t, 0.0) for t in new_long}
                crowd = crowding_scores(new_long, close_adj_values, tidx, date_i)
                adj = {t: sector_w[t] * (1.0 - crowd.get(t, 0.0)) for t in new_long}
                tot = sum(adj.values())
                if tot <= 0:
                    tot = 1.0
                    adj = {t: sector_w[t] for t in new_long}
                    tot = sum(adj.values()) or 1.0
                avg_crowding_long = float(np.mean(list(crowd.values()))) if crowd else 0.0
                for t in new_long:
                    ti = tidx[t]
                    cp = prices_v[date_i, ti]
                    if np.isnan(cp) or cp <= 0:
                        continue
                    dollar_per_name = long_pool * (adj[t] / tot)
            ...
            avg_crowding_short = None
            if new_short:
                sector_w = {t: short_weights_full.get(t, 0.0) for t in new_short}
                crowd = crowding_scores(new_short, close_adj_values, tidx, date_i)
                adj = {t: sector_w[t] * (1.0 - crowd.get(t, 0.0)) for t in new_short}
                tot = sum(adj.values())
                if tot <= 0:
                    adj = {t: sector_w[t] for t in new_short}
                    tot = sum(adj.values()) or 1.0
                avg_crowding_short = float(np.mean(list(crowd.values()))) if crowd else 0.0
                for t in new_short:
                    ti = tidx[t]
                    cp = prices_v[date_i, ti]
                    if np.isnan(cp) or cp <= 0:
                        continue
                    dollar_per_name = short_pool * (adj[t] / tot)
                    adv = dollar_volume.values[date_i, ti]
                    cap_dollar = adv * MAX_ADV_PCT if not np.isnan(adv) else dollar_per_name
            ...
            crowding_disclosure.append({"date": str(date.date()), "avg_crowding_long": avg_crowding_long,
                                        "avg_crowding_short": avg_crowding_short})
    ...
    return pv, tl, trl, crowding_disclosure
...
def main():
    ...
    for level in [100_000, 1_000_000, 10_000_000]:
        ...
        if level == 100_000:
            ...
            with (RESULTS_DIR / "crowding_disclosure.json").open("w", encoding="utf-8") as f:
    ...
    cond1 = m100k["sharpe"] >= HYP079_SHARPE_100K
    cond2 = m100k["max_drawdown"] >= HYP079_MAXDD_100K
    cond3 = sharpe_ex2020_100k >= HYP079_SHARPE_EX2020_100K
    cond4 = kurt_100k < KURTOSIS_TOLERANCE
    overall = "PASSED" if (cond1 and cond2 and cond3 and cond4) else "FAILED"

    print("=== SLUTBEDOMNING (avgorande $100k-niva, domt mot HYP-079:s egna tal) ===")
    print(f"  Villkor 1 (Sharpe >= {HYP079_SHARPE_100K}): {m100k['sharpe']:.4f} -> {'PASS' if cond1 else 'FAIL'}")
    print(f"  Villkor 2 (MaxDD >= {HYP079_MAXDD_100K:.2%}): {m100k['max_drawdown']:.2%} -> {'PASS' if cond2 else 'FAIL'}")
    print(f"  Villkor 3 (Sharpe ex-2020 >= {HYP079_SHARPE_EX2020_100K}): {sharpe_ex2020_100k:.4f} -> {'PASS' if cond3 else 'FAIL'}")
