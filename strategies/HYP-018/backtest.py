"""
HYP-018: Per-position stop-loss pa HYP-017 (idiosynkratisk vol + SPY-krasch-
overlay, PASSED, K=16).

Se research/hypothesis_registry/HYP-018-stop-loss-pa-krasch-overlay.yaml
for det lasta kriteriet, och research/strategy_specs/HYP-018-spec.md for
den fulla specen.

DIAGNOS (granskning 2026-07-30, INNAN denna hypotes lastes): dekomponering
av HYP-017:s drawdown-historik i separata episoder hittade en kvarvarande
risk-lucka krasch-overlayen INTE tacker - koncentrerade enskilda-namn-
forluster utan samtidig bred SPY-nedgang. Konkret exempel: kvartalsutgangen
2020-06-30 exekverade flera regionbanks-positioner (BHLB -65.6%, EBSB
-41.7%, HTBK -34.0%, GWB -33.0%, BRKL_old -30.9%) medan SPY:s egen
10-dagars-avkastning aldrig gick under -4.25% under samma period -
krasch-overlayen (som bara bevakar SPY) triggade aldrig, och positionerna
red hela kvartalet till ombalanseringen innan de saldes.

TILLAGG mot HYP-017 (allt annat - signal, universum, friktion, hedge,
kapacitetsspärr, krasch-overlayen, de tre motorfixarna - AR OFORANDRAT):
  Dagligt per-position stop-loss: om en enskild positions avkastning sedan
  koptillfallet (pa adjusted_close) faller under -20%, stangs HELA den
  positionen (till skillnad fran krasch-overlayens PARTIELLA, PORTFOLJBREDA
  nedskarning - stop-loss ar riktat mot EN position i taget). Aterkop
  sker INTE automatiskt - forst vid nasta ordinarie kvartalsvisa
  decil-ombalansering (samma asymmetriska monster som krasch-overlayen).
"""
...
# Per-position stop-loss (ny i HYP-018, se pass_fail_criterion - lockad, ej fri parameter)
STOP_LOSS_THRESHOLD = -0.20  # stang HELA positionen om avkastning sedan kop < -20%
...
def run_backtest(close, close_adj, hedge, vol_df, beta_df, spread_df, volume,
    ...
    for date_i in range(start_idx, len(close.index)):
        ...
        # ── Per-position stop-loss (ny, HYP-018): DAGLIG kontroll, EFTER
        # krasch-overlayen (pa vad som aterstar av varje position efter en
        # eventuell portfoljbred nedskarning samma dag). Stanger HELA
        # positionen - riktat mot enskilda namn, inte hela portfoljen -
        # om avkastningen sedan kop (pa adjusted_close) faller under
        # STOP_LOSS_THRESHOLD. Inget aterkop har - bara vid nasta
        # ordinarie kvartalsvisa ombalansering.
        for t in list(holdings.keys()):
            ti = tidx[t]
            cp = prices_v[date_i, ti]
            if np.isnan(cp):
            ...
            if ret_since_entry < STOP_LOSS_THRESHOLD:
                ...
                ret = proceeds / holdings[t]["cost"] - 1
                trade_log.append({"date": date, "ticker": t, "ret": ret,
                                   "gross_ret": ret_since_entry, "type": "stop_loss_exit",
                                   "cost": holdings[t]["cost"]})
                del holdings[t]
