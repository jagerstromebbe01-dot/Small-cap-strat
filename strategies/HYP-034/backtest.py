"""
HYP-034: Stale-price/nolldags-filter (>12/120 dagar) fore idio-vol-
rankning i HYP-023 (sektorspecifik krasch-trigger + idiosynkratisk vol +
SPY-krasch-overlay, HYP-023 fortfarande referensimplementationen).

Se research/hypothesis_registry/HYP-034-stale-price-filter-hyp023.yaml
for det lasta kriteriet.

MOTIV (5-AI-brainstorm 2026-07-31): HYP-023:s genuina OOS-test over 2025
visade en Sharpe-nedgang som var STORRE vid hogre kapitalniva - motsatt
det kapacitetstesen skulle onska. Alla 5 externa AI:er flaggade oberoende
likviditet/mikrostruktur som trolig forklaring. Denna hypotes testar den
billigaste, mest direkta atgarden: stale/ohandlade priser (vendorn
upprepar samma stangningskurs flera dagar i rad) trycker mekaniskt ner
den matta volatiliteten utan att aktien genuint ar lagrisk - en sadan
aktie kan felaktigt hamna i den lagsta idio-vol-decilen bara for att den
inte handlades.

TILLAGG mot HYP-023 (allt annat - signal, universum, friktion, hedge,
kapacitetsspärr, SPY-krasch-overlay, sektor-krasch-trigger - AR
OFORANDRAT): ett FILTER fore idio-vol-rankningen vid varje kvartalsvis
ombalansering. For varje kandidat i manadens eligible-universum: rakna
antalet dagar med adjusted_close.pct_change() == 0.0 inom samma trailing
120-dagarsfonster som redan matar idio-vol-signalen. Om nolldagarna > 12
exkluderas kandidaten helt fran rankningen den ombalanseringen, oavsett
dess idio-vol-rank. Paverkar bara kandidatpoolen vid ombalansering -
redan hallna namn saljs INTE ut for att de blir stale mitt i kvartalet.
Sjalva idio-vol-BERAKNINGEN ar oforandrad - filtret paverkar bara VILKA
namn som far tavla om decilplatserna.
"""
...
# Stale-price/nolldags-filter (ny i HYP-034, se pass_fail_criterion -
# lockade konstanter, SAMMA fonster som VOL_LOOKBACK_DAYS)
ZERO_RETURN_WINDOW = VOL_LOOKBACK_DAYS  # 120 - samma fonster som idio-vol-signalen
ZERO_RETURN_MAX_DAYS = 12   # > 12 nolldagar av upp till 120 -> exkluderad fran rankning
...
# ════════════════════════════════════════════════════════════
#  STALE-PRICE/NOLLDAGS-FILTER (ny i HYP-034)
# ════════════════════════════════════════════════════════════
def compute_zero_return_days(close_adj, window):
    """Rullande antal dagar med adjusted_close.pct_change() == 0.0 (genuint
    noll - tecken pa att aktien inte handlades och vendorn upprepade forra
    stangningskursen) inom trailing `window` handelsdagar. NaN-dagar (ingen
    prisserie) raknas INTE som nolldagar (NaN == 0.0 ar False), och raden
    forblir NaN tills fonstret har `window` giltiga observationer - samma
    min_periods-beteende som idio-vol-signalens egen .rolling(window).std()."""
    ret = close_adj.pct_change()
    is_zero = (ret == 0.0).astype("float32")
    return is_zero.rolling(window).sum()
...
def run_backtest(close, close_adj, hedge, vol_df, beta_df, spread_df, volume,
                 universe_by_month, capital_level: float, bank_flags: dict, zero_days_df):
    """
    ...
    for date_i in range(start_idx, len(close.index)):
        ...
        if date in rebal_set:
            ...
            zero_days_today = zero_days_df.loc[date]
            # STALE-PRICE/NOLLDAGS-FILTER (ny i HYP-034): fore rankning,
            # exkludera kandidater med > ZERO_RETURN_MAX_DAYS nolldagar i
            # samma 120-dagarsfonster som idio-vol-signalen. Rakans separat
            # fran pre_filter (som redan har giltig vol+pris) for att kunna
            # logga hur manga som faktiskt filtreras bort - diagnostik (a)
            # i pass_fail_criterion, gatar INTE PASS/FAIL.
            pre_filter = []
            for t in eligible:
                v = vol_today[t]
                cp = prices_v[date_i, tidx[t]]
                if not np.isnan(v) and not np.isnan(cp) and cp > 0:
                    pre_filter.append((v, t, cp))
            scores = []
            n_stale_excluded = 0
            for v, t, cp in pre_filter:
                zd = zero_days_today[t]
                if not np.isnan(zd) and zd > ZERO_RETURN_MAX_DAYS:
                    n_stale_excluded += 1
                    continue
            ...
            stale_filter_log.append({"date": date, "n_pre_filter": len(pre_filter),
                                      "n_stale_excluded": n_stale_excluded,
                                      "n_eligible_post_filter": len(scores), "n_top": n_top})
    ...
    sfl = pd.DataFrame(stale_filter_log)
    return pv, tl, stl, sfl
...
def run_for_capital_level(capital_level, close, close_adj, hedge, vol_df, beta_df, spread_df, volume,
                           universe_by_month, bank_flags, zero_days_df):
    print(f"    Kör backtest (kapitalnivå {capital_level:,.0f})...")
    pv, tl, stl, sfl = run_backtest(close, close_adj, hedge, vol_df, beta_df, spread_df, volume, universe_by_month,
                                     capital_level, bank_flags, zero_days_df)
    ...
    result = {
        ...
        # Diagnostik (a) i HYP-034:s pass_fail_criterion - gatar INTE PASS/FAIL.
        "avg_stale_excluded_per_rebal": float(sfl["n_stale_excluded"].mean()) if len(sfl) else 0.0,
        "avg_n_top_post_filter": float(sfl["n_top"].mean()) if len(sfl) else 0.0,
        "avg_n_pre_filter": float(sfl["n_pre_filter"].mean()) if len(sfl) else 0.0,
    }
    ...
    sfl.to_csv(RESULTS_DIR / f"stale_filter_log_{int(capital_level)}.csv", index=False)
...
def main():
    ...
    print(f"Beräknar nolldags-filter (trailing {ZERO_RETURN_WINDOW} dagar, tröskel >{ZERO_RETURN_MAX_DAYS})...")
    zero_days_df = compute_zero_return_days(close_adj, ZERO_RETURN_WINDOW)
    ...
    for level in levels:
        ...
        print(f"    CAGR={result['cagr']:+.2%}  Sharpe={result['sharpe']:.2f}  "
              ...
              f"AvgStaleExkl/rebal={result['avg_stale_excluded_per_rebal']:.1f}  "
              f"AvgDecilstorlek={result['avg_n_top_post_filter']:.1f}\n")
