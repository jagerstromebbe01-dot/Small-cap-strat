"""
HYP-088: Havstang (3,0x, gating) med marginalanrop/tvingad nedskalning
som overlay pa HYP-081:s redan kombinerade portfolj.

Se research/hypothesis_registry/HYP-088-havstang-3x-margin-anrop-hyp081.yaml
for det lasta kriteriet. IDENTISK marginalanrops-mekanism som HYP-059/
065/087 - ENDA skillnaden ar gating-nivan (3,0x istallet for 2,5x).

KALLOR (aterananvander direkt, ingen omberkning):
  strategies/HYP-081/results/portfolio_value_combined_{level}.csv
  strategies/HYP-081/results/portfolio_value_oos2025_combined_{level}.csv
"""
...
HYP081_RESULTS = STRATEGIES_ROOT / "HYP-081" / "results"
...
RF_ANNUAL = 0.02
GATING_LEVERAGE = 3.0
DISCLOSURE_LEVERAGES = [1.5, 2.0, 2.5]
GATING_SPREAD = 0.015
...
# HYP-081:s egna lasta capital_level_results (CAGR vid $100k/$1M/$10M)
HYP081_MAIN_REF_CAGR = {100_000: 0.078748, 1_000_000: 0.077408, 10_000_000: 0.073023}

# HYP-087:s egna 2,5x-gating-tal ($100k/$1M/$10M), for jamforelse-disclosure.
HYP087_25X_SHARPE = {100_000: 1.1631, 1_000_000: 1.1340, 10_000_000: 1.0848}
HYP087_25X_CAGR = {100_000: 0.1431, 1_000_000: 0.1395, 10_000_000: 0.1283}
HYP087_25X_MAXDD = {100_000: -0.1159, 1_000_000: -0.1159, 10_000_000: -0.1129}

DSR_N_TRIALS = 85  # k_total_hypotheses_before_this (84) + 1


def load_hyp081_combined(level: int) -> pd.Series:
    return pd.read_csv(HYP081_RESULTS / f"portfolio_value_combined_{level}.csv",
                        index_col=0, parse_dates=True)["portfolio_value"]


def load_hyp081_oos(level: int) -> pd.Series:
    return pd.read_csv(HYP081_RESULTS / f"portfolio_value_oos2025_combined_{level}.csv",
                        index_col=0, parse_dates=True)["portfolio_value"]
...
def main():
    ...
    print("=== HYP-088: havstang 3,0x (gating) med marginalanrop-overlay pa HYP-081 ===\n")

    all_results = {}
    for level in levels:
        pv_main = load_hyp081_combined(level)
        pv_oos = load_hyp081_oos(level)
        ...
        print(f"  ${level:>10,.0f}  [GATING 3,0x, konservativ spread] Sharpe={gating['sharpe']:.4f}  "
        ...
        print(f"              JAMFORELSE mot HYP-087s 2,5x: Sharpe {gating['sharpe']:.4f} vs "
              f"{HYP087_25X_SHARPE[level]:.4f}  CAGR {gating['cagr']:+.2%} vs {HYP087_25X_CAGR[level]:+.2%}  "
              f"MaxDD {gating['max_drawdown']:.2%} vs {HYP087_25X_MAXDD[level]:.2%}")

        print("              -- disclosure (1,5x/2,0x/2,5x, konservativ spread) --")
        for lev in DISCLOSURE_LEVERAGES:
            d = next(r for r in level_results if r["leverage"] == lev and r["spread_label"] == "konservativ_gating")
            print(f"              {lev}x: Sharpe={d['sharpe']:.4f}  CAGR={d['cagr']:+.2%}  "
                  f"MaxDD={d['max_drawdown']:.2%}  Calmar={d['calmar']:.3f}  "
                  f"#marginalanrop={d['n_margin_calls_main']}")
        print()

    lvl = 100_000
    gating_100k = next(r for r in all_results[lvl] if r["leverage"] == GATING_LEVERAGE and r["spread_label"] == "konservativ_gating")
    ref_cagr = HYP081_MAIN_REF_CAGR[lvl]
    ...
    print("=== SLUTBEDOMNING (avgorande $100k-niva, 3,0x hafstang, konservativ spread) ===")
    print(f"  Villkor 1 (Sharpe >= 1,0): {gating_100k['sharpe']:.4f} -> {'PASS' if cond1 else 'FAIL'}")
    print(f"  Villkor 2 (CAGR > HYP-081 {ref_cagr:.2%} + 5,0pp = {ref_cagr + 0.05:.2%}): "
    ...
    summary = {
        ...
        "all_scenarios_by_level": all_results, "hyp081_main_ref_cagr": HYP081_MAIN_REF_CAGR,
