...
HYP-032: Traditionell Kelly-sizing med signalstyrka INOM den redan valda
decilen - ENDA andringen mot HYP-023 (som likaviktar). EXPLICIT
CEO-BESLUT att ateroppna Ejay/Kelly-mekanismen for ett sista test (se
research/hypothesis_registry/HYP-032-kelly-sizing-signalstyrka.yaml).
FORVANTAD UTGANG: FAIL (samma mekanism-typ som dodade 7 Ejay-varianter),
men aldrig testad pa just idio-vol-decilen tidigare.
"""
...
def run_backtest(close, close_adj, hedge, vol_df, beta_df, spread_df, volume,
    ...
    for date_i in range(start_idx, len(close.index)):
        ...
        if date in rebal_set:
            ...
            # ── KELLY-VIKTNING (ny, HYP-032): z-score mot HELA det behöriga
            # universumet (INTE bara decilen), edge = max(0, -z) (lagre
            # idio-vol = starkare signal), vikt PROPORTIONELL MOT edge/varians -
            # traditionell Kelly-struktur. Tak pa 3x likaviktad andel,
            # normaliserat. Se pass_fail_criterion for exakt formel, lockad
            # INNAN nagon backtest kordes.
            kelly_weights = {}
            if scores:
                eligible_vols = np.array([v for v, _, _ in scores if not np.isnan(v)])
                cs_mean = float(eligible_vols.mean())
                cs_std = float(eligible_vols.std())
                for v, t, _ in scores[:n_top]:
                    z = (v - cs_mean) / cs_std if cs_std > 0 else 0.0
                    edge = max(0.0, -z)
                    var = v * v
                    kelly_weights[t] = edge / var if var > 0 else 0.0

                total_w = sum(kelly_weights.values())
                if total_w > 0:
                    for t in kelly_weights:
                        kelly_weights[t] /= total_w
                else:
                    for t in kelly_weights:
                        kelly_weights[t] = 1.0 / len(kelly_weights)

                equal_w = 1.0 / len(kelly_weights) if kelly_weights else 0.0
                cap_w = 3.0 * equal_w
                for t in kelly_weights:
                    kelly_weights[t] = min(kelly_weights[t], cap_w)
                total_w2 = sum(kelly_weights.values())
                if total_w2 > 0:
                    for t in kelly_weights:
                        kelly_weights[t] /= total_w2
            ...
            if new_names:
                # HYP-032: Kelly-relativa vikter BLAND de nya namnen (samma
                # forenkling som HYP-023:s likaviktade version - befintliga
                # positioner omviktas inte lopande, bara NYA namns andel av
                # tillgangligt cash paverkas).
                new_weight_sum = sum(kelly_weights.get(t, 0.0) for t in new_names)
                for t in new_names:
                    ti = tidx[t]
                    cp = prices_v[date_i, ti]
                    if np.isnan(cp) or cp <= 0:
                        continue
                    rel_w = kelly_weights.get(t, 0.0) / new_weight_sum if new_weight_sum > 0 else 1.0 / len(new_names)
                    target_dollar_per_name = cash * rel_w
