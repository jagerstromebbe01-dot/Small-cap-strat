"""
HYP-021: Idiosynkratisk volatilitet residualiserad mot ett small-cap-
kompositfaktor i stallet for SPY (signalrenhets-forfining av HYP-017,
PASSED, K=16/19 - Sharpe 0.72/0.75/0.72).

Se research/hypothesis_registry/HYP-021-idio-vol-smallcap-faktor.yaml
for det lasta kriteriet, och research/strategy_specs/HYP-021-spec.md for
den fulla specen.

MOTIVERING: "idiosynkratisk" bor rimligen betyda "det som inte forklaras
av den egna peer-gruppens gemensamma rorelse" - men HYP-015/017
residualiserar mot SPY, ett stor-cap-dominerat index, inte mot small-cap-
bolagens egen peer-grupp. Denna hypotes ersatter SPY med ett likaviktat
medelvarde av HELA small-cap-universumets dagliga avkastningar SOM
REFERENS FOR SIGNALEN - samma typ av "gratis" forbattring (ingen ny risk)
som redan fungerade en gang (HYP-014 total-vol -> HYP-015 idio-vol).

VIKTIGT: krasch-overlayen och portfoljens FAKTISKA hedge ar HELT
OFORANDRADE - fortfarande SPY (ett riktigt, blankningsbart instrument;
det finns inget "small-cap-index" att faktiskt hedga med i verkligheten).
Bara SIGNALENS referensserie byts - tva separata beta-berakningar
(en mot SPY for hedgen, en mot small-cap-kompositfaktorn for signalen).
"""
...
# ════════════════════════════════════════════════════════════
#  SMALL-CAP-KOMPOSITFAKTOR (ny i HYP-021) - anvands ENDAST som referens
#  for signalens beta/residual-berakning, ALDRIG som hedgeinstrument
#  (portfoljens faktiska hedge ar fortfarande SPY, oforandrad).
# ════════════════════════════════════════════════════════════
def compute_smallcap_factor(close_adj):
    """
    Likaviktat medelvarde av dagliga avkastningar over HELA small-cap-
    panelen, uttryckt som en syntetisk "prisserie" (kumulativ produkt av
    1+avkastning) sa den ar drop-in-kompatibel med compute_beta() och
    compute_idiosyncratic_vol(), som bada forvantar sig en prisserie och
    raknar pct_change() internt - ingen andring behovs i de funktionerna.
    """
    daily_ret = close_adj.pct_change().mean(axis=1).fillna(0.0)
...
def main():
    ...
    print("Estimerar beta mot SPY (for portfoljens FAKTISKA hedge, oforandrad fran HYP-017)...")
    beta_df = compute_beta(close_adj, hedge, BETA_WINDOW)

    print("Bygger small-cap-kompositfaktor och estimerar beta mot DEN (bara for signalen)...")
    smallcap_factor = compute_smallcap_factor(close_adj)
    beta_signal = compute_beta(close_adj, smallcap_factor, BETA_WINDOW)

    print("Beräknar idiosynkratisk-volatilitets-signal (trailing 120 dagar, mot small-cap-faktorn)...")
