"""
HYP-038: Likviditetsviktad (roten-ur-ADV) allokering inom decilen pa
HYP-037 (idiosynkratisk vol + SPY-krasch-overlay med villkorat
aterintrade + sektor-krasch-trigger, HYP-037 referensimplementationen).

Se research/hypothesis_registry/HYP-038-likviditetsviktad-allokering-hyp037.yaml
for det lasta kriteriet.

TILLAGG mot HYP-037 (allt annat - signal, universum, friktion, hedge,
kapacitetsspärr, GLOBAL rankning, SPY-krasch-overlay MED dess villkorade
aterintrade, sektor-krasch-trigger - AR OFORANDRAT): dollarallokeringen
over NYA positioner vid ombalansering byts fran likaviktat till
likviditetsviktat (roten ur 20-dagars dollar-ADV, INTE linjart).
Kapacitetsspärren (10%/5% under restriktion) forblir ett HART TAK per
namn, tillampad EFTER viktberakningen - oforandrad ordning och varden.
"""
...
# Villkorat overlay-aterintrade (ny i HYP-037, se pass_fail_criterion -
# lockade siffror, ATERANVANDA fran AI-forslagen istallet for nya)
RECOVERY_FRACTION = 0.50          # SPY maste aterhamta minst halva det utlosande fallet
SPREAD_MEDIAN_WINDOW = 252         # rullande handelsdagar for spread-normaliseringens median
RESTRICTED_MAX_ADV_PCT = 0.05      # halverad kapacitetsspärr for NYA positioner under restriktion
...
# ════════════════════════════════════════════════════════════
#  LIKVIDITETSGAUGE FOR VILLKORAT OVERLAY-ATERINTRADE (ny i HYP-037,
#  SAMMA anti-cirkularitetsprincip som bankkompositen ovan: manadens
#  HELA eligible-universum, INTE bara hallna namn - annars skulle en
#  portfolj som redan flytt de sammaste spreadarna trivialt visa "lag
#  spread" oavsett verkliga marknadsforhallanden)
# ════════════════════════════════════════════════════════════
def compute_universe_avg_spread(spread_df, universe_by_month, date_index):
    month_keys = sorted(universe_by_month.keys())
    month_dates = pd.to_datetime(month_keys)
    day_month_idx = month_dates.searchsorted(date_index, side="right") - 1
    day_month_idx = np.clip(day_month_idx, 0, len(month_dates) - 1)

    avg_spread = pd.Series(index=date_index, dtype="float64")
    for mi in np.unique(day_month_idx):
        mask = day_month_idx == mi
        mk = month_keys[mi]
        eligible = [t for t in universe_by_month.get(mk, []) if t in spread_df.columns]
        if eligible:
            avg_spread.iloc[mask] = spread_df.loc[date_index[mask], eligible].mean(axis=1).values
    return avg_spread
...
def run_backtest(close, close_adj, hedge, vol_df, beta_df, spread_df, volume,
    ...
    # VILLKORAT OVERLAY-ATERINTRADE (ny i HYP-037): likviditetsgauge
    # (universumbrett genomsnittligt Corwin-Schultz-spread, se
    # compute_universe_avg_spread) + dess rullande 252-dagars median.
    # capacity_restricted ar SANT fran och med en SPY-krasch-overlay-
    # trigger tills BADA aterhamtnings-/normaliseringsvillkoren uppfylls
    # SAMTIDIGT - se pass_fail_criterion.
    universe_avg_spread = compute_universe_avg_spread(spread_df, universe_by_month, close.index)
    spread_median_v = universe_avg_spread.rolling(SPREAD_MEDIAN_WINDOW, min_periods=SPREAD_MEDIAN_WINDOW).median().values
    ...
    for date_i in range(start_idx, len(close.index)):
        ...
        if date in rebal_set:
            ...
            if new_names:
                # LIKVIDITETSVIKTAD ALLOKERING (HYP-038): vikt per namn
                # PROPORTIONELL mot roten ur 20-dagars dollar-ADV, INTE
                # likaviktat. Saknad/noll ADV for ett enskilt namn ersatts
                # med MEDELVARDET av gruppens ovriga giltiga roten-ur-ADV-
                # vikter (INTE noll). Om SAMTLIGA saknar giltig ADV: ren
                # likaviktning (samma fallback som HYP-023/037).
                adv_map = {t: dollar_volume.values[date_i, tidx[t]] for t in new_names}
                sqrt_adv = {t: np.sqrt(v) for t, v in adv_map.items() if not np.isnan(v) and v > 0}
                if sqrt_adv:
                    fallback_weight = float(np.mean(list(sqrt_adv.values())))
                    raw_weights = {t: sqrt_adv.get(t, fallback_weight) for t in new_names}
                else:
                    raw_weights = {t: 1.0 for t in new_names}
                total_weight = sum(raw_weights.values())
                weight_map = {t: w / total_weight for t, w in raw_weights.items()}
                # VIKTIGT: malbeloppet maste rakas mot en FRUSEN ogonblicksbild
                # av cash FORE loopen - INTE den lopande `cash`-variabeln, som
                # krymper for varje kop. Att anvanda den lopande `cash` skulle
                # gora hela allokeringen beroende av i vilken ORDNING new_names
                # racikas upp (som i sin tur varierar mellan processkorningar,
                # eftersom target_tickers ar en Python-mangd med
                # hash-randomiserad iterationsordning) - ett icke-deterministiskt
                # resultat, upptackt genom att samma korning gav olika MaxDD tre
                # ganger i rad innan denna fix.
                rebalance_cash = cash
                ...
                for t in new_names:
                    ...
                    if np.isnan(cp) or cp <= 0:
                        continue
                    target_dollar_per_name = weight_map[t] * rebalance_cash
        ...
        if not np.isnan(spy10) and spy10 < CRASH_TRIGGER_RET and not haircut_active:
            # VILLKORAT ATERINTRADE (HYP-037): en NY episod satter
            # capacity_restricted och sparar start-/bottenpris for
        ...
        # ── Villkorat overlay-aterintrade (ny, HYP-037): DAGLIG kontroll,
        # HELT OBEROENDE av haircut_active ovan - capacity_restricted kan
        # forbli SANT langt efter att haircut_active redan blivit FALSKT
        # (haircut_active styr bara om en NY nedskarning far triggas,
        # capacity_restricted styr NAR full kapacitet far aterupptas for
        # NYA positioner). Aterstalls ENDAST nar BADA villkoren galler
        # SAMTIDIGT samma dag.
        if capacity_restricted and crash_trough_price is not None:
            ...
            if recovered_enough and liquidity_normalized:
                capacity_restricted = False
                if capacity_restriction_log and capacity_restriction_log[-1]["end_date"] is None:
                    capacity_restriction_log[-1]["end_date"] = date
    ...
    crl = pd.DataFrame(capacity_restriction_log)
    return pv, tl, stl, crl
...
def run_for_capital_level(capital_level, close, close_adj, hedge, vol_df, beta_df, spread_df, volume,
                           universe_by_month, bank_flags):
    print(f"    Kör backtest (kapitalnivå {capital_level:,.0f})...")
    pv, tl, stl, crl = run_backtest(close, close_adj, hedge, vol_df, beta_df, spread_df, volume, universe_by_month,
                                     capital_level, bank_flags)

    if len(crl):
        crl["duration_days"] = (pd.to_datetime(crl["end_date"].fillna(pv.index[-1])) -
                                 pd.to_datetime(crl["start_date"])).dt.days
        avg_restriction_days = float(crl["duration_days"].mean())
    else:
        avg_restriction_days = 0.0
    ...
    result = {
        ...
        "n_capacity_restriction_episodes": len(crl),
        "avg_capacity_restriction_days": avg_restriction_days,
    }
    ...
    crl.to_csv(RESULTS_DIR / f"capacity_restriction_log_{int(capital_level)}.csv", index=False)
...
def main():
    ...
    for level in levels:
        ...
        result = run_for_capital_level(level, close, close_adj, hedge, vol_df, beta_df, spread_df, volume,
                                        universe_by_month, bank_flags)
        ...
        print(f"    CAGR={result['cagr']:+.2%}  Sharpe={result['sharpe']:.2f}  "
              ...
              f"KapRestrEpisoder={result['n_capacity_restriction_episodes']}  "
              f"SnittRestrDagar={result['avg_capacity_restriction_days']:.0f}\n")
