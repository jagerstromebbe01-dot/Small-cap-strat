"""
HYP-090: Crypto tvärsnitts-momentum (omvänt tecken mot HYP-089) på
30-dagars utjämnad OLS-residual, MÅNADSVIS ombalansering.

Se research/hypothesis_registry/HYP-090-crypto-tvarsnitt-momentum-manadsvis.yaml
för det fullständigt låsta kriteriet. Universum-, kapacitets- och
friktionslogik kopierad OFÖRÄNDRAD från strategies/HYP-089/backtest.py
(inte importerad - per projektets konvention kopieras hypotesspecifik
kod, den redigeras inte i den ursprungliga mappen). Två låsta ändringar
mot HYP-089: riktningen omvänd, signalen är ett 30-dagars glidande
snitt av den dagliga residualen (istället för enskild dag) och
ombalanseringen är månadsvis (istället för veckovis).
"""
...
]  # IDENTISK kandidatlista som HYP-089 (UNI/MATIC exkluderade, stale data)
...
VOLUME_WINDOW = 90
BETA_WINDOW_DAYS = 90
MIN_PAIRS_FOR_OLS = 60
RESIDUAL_AVG_WINDOW_DAYS = 30  # CEO-LÅST, nytt mot HYP-089
MIN_RESIDUAL_DAYS = 20  # CEO-LÅST golv (>=20 av 30 dagar), se kriteriet
MIN_UNIVERSE_SIZE = 6
MAX_ADV_PCT = 0.10
UNIVERSE_REVIEW_FREQ = "QE"
DECISION_FREQ = "ME"  # CEO-LÅST: månadsvis istället för HYP-089:s "W"
...
DSR_N_TRIALS = 87  # k_total_hypotheses_before_this (86) + 1


# ───────────────────────── datainladdning (identisk med HYP-089) ─────────────────────────
...
# ───────────────────────── signal: 30-dagars snitt av daglig OLS-residual ─────────────────────────

def compute_daily_residuals(ret, tickers, btc_col="BTC", window=f"{BETA_WINDOW_DAYS}D", min_periods=MIN_PAIRS_FOR_OLS):
    """Vektoriserad rullande OLS (kalenderdagsbaserat fönster) - beta/alpha
    via rullande moment (Cov/Var), INTE en Python-loop per dag (samma
    optimeringsprincip som redan etablerad i strategies/common/friction.py::
    corwin_schultz_spread - numeriskt ekvivalent med en per-dag-OLS-loop,
    men praktiskt körbar på ~28 tickers x ~5600 dagar)."""
    x = ret[btc_col]
    mean_x = x.rolling(window, min_periods=min_periods).mean()
    mean_xx = (x * x).rolling(window, min_periods=min_periods).mean()
    var_x = mean_xx - mean_x ** 2

    residuals = pd.DataFrame(index=ret.index, dtype=float)
    for tk in tickers:
        if tk == btc_col:
            continue
        y = ret[tk]
        mean_y = y.rolling(window, min_periods=min_periods).mean()
        mean_xy = (x * y).rolling(window, min_periods=min_periods).mean()
        cov_xy = mean_xy - mean_x * mean_y
        beta = cov_xy / var_x
        alpha = mean_y - beta * mean_x
        residuals[tk] = y - (alpha + beta * x)
    return residuals


def compute_residual_avg(daily_residuals, window=f"{RESIDUAL_AVG_WINDOW_DAYS}D", min_periods=MIN_RESIDUAL_DAYS):
    return daily_residuals.rolling(window, min_periods=min_periods).mean()


def tertile_split(scores):
    """Returnerar (bottom_third, top_third) - ANROPANDE KOD avgör vilken
    som är long/short (HYP-090 vänder detta mot HYP-089, se run_backtest)."""
    ...
    if n_third < 1:
        return [], []
    bottom = [t for t, _ in ordered[:n_third]]
    top = [t for t, _ in ordered[-n_third:]]
    return bottom, top


# ───────────────────────── backtest-motor ─────────────────────────

def run_backtest(close, high, low, dollar_vol, residual_avg, schedule, idx, capital_level, start_date, end_date):
    ...
    for d in trading_days:
        if d in decision_dates:
            ...
            scores = {tk: residual_avg.at[d, tk] for tk in universe
                      if d in residual_avg.index and tk in residual_avg.columns and pd.notna(residual_avg.at[d, tk])}
            bottom_third, top_third = tertile_split(scores)
            # RIKTNING OMVÄND mot HYP-089 (CEO-LÅST): long ÖVRE tredjedelen
            # (mest positiv residual - "fortsättning"), short NEDRE.
            new_long, new_short = top_third, bottom_third
            ...
            monthly_log.append({
                "date": d, "n_universe": len(universe), "n_scored": len(scores),
    ...
    return pv, pd.DataFrame(monthly_log)
...
def main():
    print("=== HYP-090: Crypto tvärsnitts-momentum (omvänt tecken), 30d-snitt residual, månadsvis ===\n")
    ...
    print("Beräknar rullande dagliga OLS-residualer (90d-fönster) och 30d-snitt...")
    daily_residuals = compute_daily_residuals(ret, CANDIDATES)
    residual_avg = compute_residual_avg(daily_residuals)
    print("Klart.\n")

    first_8 = next((d for d, u in schedule if len(u) >= 8), None)
    main_start = first_8 if first_8 is not None else next(d for d, u in schedule if len(u) >= MIN_UNIVERSE_SIZE)
    print(f"Huvudperiod startar {main_start.date()} (samma datum som HYP-089)\n")
    ...
    for level in levels:
        pv_main, log_main = run_backtest(close, high, low, dollar_vol, residual_avg, schedule, idx,
                                          level, main_start, MAIN_END)
        pv_oos, log_oos = run_backtest(close, high, low, dollar_vol, residual_avg, schedule, idx,
        ...
        log_main.to_csv(RESULTS_DIR / f"monthly_log_main_{level}.csv", index=False)
        ...
        disclosures.append({
            ...
            "avg_n_scored": float(log_main["n_scored"].mean()),
            "avg_n_long": float(log_main["n_long"].mean()),
            "avg_n_short": float(log_main["n_short"].mean()),
            "pct_months_below_floor": float((log_main["n_scored"] < MIN_UNIVERSE_SIZE).mean()) if len(log_main) else None,
            "n_rebalances": len(log_main),
        })

        cond1 = sh > 1.00
        cond2 = md >= -0.25
        overall = "PASS" if (cond1 and cond2) else "FAIL"
        print(f"  ${level:>10,.0f}  Sharpe={sh:.4f} (>1.00: {'PASS' if cond1 else 'FAIL'})  "
        ...
        print(f"               snitt bruttoexp: long={disclosures[-1]['avg_gross_long']:.3f} "
              f"short={disclosures[-1]['avg_gross_short']:.3f}  antal ombalanseringar={disclosures[-1]['n_rebalances']}  "
              f"snitt antal mynt: universum={disclosures[-1]['avg_n_universe']:.1f} "
              f"scorade={disclosures[-1]['avg_n_scored']:.1f} "
              f"long={disclosures[-1]['avg_n_long']:.1f} short={disclosures[-1]['avg_n_short']:.1f}\n")

    cond1_100k = main_results[0]["sharpe"] > 1.00
    ...
    print(f"  Villkor 1 (Sharpe > 1,00): {main_results[0]['sharpe']:.4f} -> {'PASS' if cond1_100k else 'FAIL'}")
