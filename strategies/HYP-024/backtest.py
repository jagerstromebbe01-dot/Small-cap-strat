"""
HYP-024: Rorelselonsamhet (OperatingIncomeLoss/Assets), topp-decilen,
ra signal utan krasch-skydd. Forsta hypotesen i en NY faktorfamilj
(kvalitet/lonsamhet) - INTE en vidareutveckling av HYP-017/023-linjen
(idiosynkratisk vol/SPY-krasch-overlay).

Se research/hypothesis_registry/HYP-024-rorelselonsamhet-decil.yaml for
det lasta kriteriet, och research/strategy_specs/HYP-024-spec.md for
den fulla specen.

Aterananvander HYP-017:s motor (SPY-beta-hedge, friktion, kapacitetsspärr,
de tre motorfixarna - snappade ombalanseringsdatum, adjusted_close
genomgaende, flag_implausible_liquidity) - INTE HYP-014:s aldre kod, som
foregick dessa fixar. TAR BORT idiosynkratisk-vol-signalen och HELA
SPY-krasch-overlayen (ra signal-test, samma ordning som idio-vol
testades: HYP-015 ra -> HYP-017 med riskverktyg lagt pa EFTER lofte).

DATA: data/cache/profitability_by_ticker.jsonl (byggd av
data/fetch_profitability_history.py) - senaste FILED 10-K:s
OperatingIncomeLoss/Assets per bolag, undviker framatblick genom att
bara anvanda varden filade FORE respektive ombalanseringsdatum.

UNIVERSUM: bank-/finansklassificerade namn EXKLUDERAS helt (samma
sector.py-metod som HYP-022/023) - rorelseresultat ar inte ett
jamforbart begrepp for banker/REITs.
"""
...
MIN_HISTORY_DAYS = 130    # samma minsta uppvarmningsperiod som HYP-014/015/017

DECILE_FRACTION = 0.10    # topp-decilen av rorelselonsamhet
...
# ════════════════════════════════════════════════════════════
#  LONSAMHETSSIGNAL (ny i HYP-024)
# ════════════════════════════════════════════════════════════
def load_profitability_data() -> dict:
    """Per ticker: sorterad lista av (filed_date_str, opinc/assets-kvot),
    en post per 10-K dar BADA OperatingIncomeLoss och Assets har SAMMA
    'end'-datum (samma bokslut). filed_date_str anvands for as-of-
    uppslagning (motorfix-liknande princip: bara varden filade FORE
    ombalanseringsdatumet ar tillgangliga, undviker framatblick)."""
    profitability = {}
    ...
    with PROFITABILITY_FILE.open(encoding="utf-8") as f:
        for line in f:
            ...
            for oi in opinc_list:
                ...
                ratio = oi["val"] / a["val"]
                filed = oi["filed"]
                observations.append((filed, ratio))
            if observations:
                observations.sort()
...
def profitability_asof(profitability: dict, ticker: str, as_of_date_str: str):
    """Senaste FILED varde strikt FORE as_of_date_str (undviker att
    anvanda ett bokslut samma dag det publicerades som om det vore kant
    fran dagens borjan)."""
...
# ════════════════════════════════════════════════════════════
#  BETA (identisk med HYP-014/017, MED sakerhetsspärren mot degenererad beta)
# ════════════════════════════════════════════════════════════
...
# ════════════════════════════════════════════════════════════
#  BACKTEST-MOTOR (samma struktur som HYP-017, UTAN krasch-overlay -
#  ra signal-test)
# ════════════════════════════════════════════════════════════
def run_backtest(close, close_adj, hedge, profitability, bank_flags, beta_df, spread_df, volume,
    ...
    for date_i in range(start_idx, len(close.index)):
        ...
        if date in rebal_set:
            ...
            # UNIVERSUM-BEGRANSNING (ny i HYP-024): bank-/finansnamn
            # exkluderas helt - rorelseresultat ar inte jamforbart for
            # banker/REITs (se pass_fail_criterion).
            eligible = [t for t in universe_by_month.get(month_key, [])
                        if t in tidx and not bank_flags.get(t, False)]
            scores = []
            for t in eligible:
                ratio = profitability_asof(profitability, t, as_of)
                cp = prices_v[date_i, tidx[t]]
                if ratio is not None and not np.isnan(cp) and cp > 0:
                    scores.append((ratio, t, cp))
            scores.sort(reverse=True)  # HOGST rorelselonsamhet forst (motsatt riktning mot low-vol/idio-vol)
...
# ════════════════════════════════════════════════════════════
#  METRICS (identiska definitioner mot HYP-014/017)
# ════════════════════════════════════════════════════════════
...
#  KÖRNING
# ════════════════════════════════════════════════════════════
def run_for_capital_level(capital_level, close, close_adj, hedge, profitability, bank_flags, beta_df, spread_df,
                           volume, universe_by_month):
    print(f"    Kör backtest (kapitalnivå {capital_level:,.0f})...")
    pv, tl = run_backtest(close, close_adj, hedge, profitability, bank_flags, beta_df, spread_df, volume,
...
def main():
    ...
    print("Laddar rörelselönsamhet (data/cache/profitability_by_ticker.jsonl)...")
    profitability = load_profitability_data()
    print(f"  {len(profitability)} av {len(tickers)} tickers har användbar lönsamhetsdata.\n")

    print("Klassificerar bank-/finansnamn (exkluderas helt)...")
    ...
    for level in levels:
        ...
        result = run_for_capital_level(level, close, close_adj, hedge, profitability, bank_flags, beta_df,
                                        spread_df, volume, universe_by_month)
