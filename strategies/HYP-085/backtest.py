"""
HYP-085: Kontinuerlig sqrt(ADV)-kapacitetsviktning (ersatter hard
ADV-sparr) pa HYP-079. Se
research/hypothesis_registry/HYP-085-sqrt-adv-viktning-hyp079.yaml for
det lasta kriteriet.

Identisk motor/signal/branschtak/positionstak som HYP-079. ENDA
ANDRINGEN: den harda 10%-ADV-sparren tas bort helt, ersatt med
kontinuerlig viktning proportionell mot sqrt(20-dagars ADV$),
kombinerad multiplikativt med branschtaksvikten.
"""
...
HYP079_CAPACITY_RATIO = 0.6936 / 0.8433
...
# ════════════════════════════════════════════════════════════
#  MOTOR (HYP-079 + sqrt(ADV)-viktning istallet for hard sparr)
# ════════════════════════════════════════════════════════════
def run_backtest(close, close_adj, spread_df, volume, filing_index, universe_by_month, sic_map,
    ...
    for date_i in range(start_idx, len(close.index)):
        ...
        if date in rebal_set:
            ...
            if new_long:
                ...
                adv_w = {}
                for t in new_long:
                    adv = dollar_volume.values[date_i, tidx[t]]
                    adv_w[t] = np.sqrt(adv) if (not np.isnan(adv) and adv > 0) else 0.0
                combined = {t: sector_w[t] * adv_w[t] for t in new_long}
                tot = sum(combined.values())
                if tot <= 0:
                    combined = sector_w
                    tot = sum(combined.values()) or 1.0
                if len(new_long) > 0:
                    max_single_position_share.append(("long", str(date.date()), max(combined.values()) / tot))
                for t in new_long:
                    ti = tidx[t]
                    cp = prices_v[date_i, ti]
                    if np.isnan(cp) or cp <= 0:
                        continue
                    dollar_per_name = long_pool * (combined[t] / tot)
                    sz = min(dollar_per_name, cash)
                    if sz <= 0:
                        continue
            ...
            if new_short:
                ...
                for t in new_short:
                    adv = dollar_volume.values[date_i, tidx[t]]
                    adv_w[t] = np.sqrt(adv) if (not np.isnan(adv) and adv > 0) else 0.0
                combined = {t: sector_w[t] * adv_w[t] for t in new_short}
                tot = sum(combined.values())
                if tot <= 0:
                    combined = sector_w
                    tot = sum(combined.values()) or 1.0
                if len(new_short) > 0:
                    max_single_position_share.append(("short", str(date.date()), max(combined.values()) / tot))
                for t in new_short:
                    ti = tidx[t]
                    cp = prices_v[date_i, ti]
                    if np.isnan(cp) or cp <= 0:
                        continue
                    dollar_per_name = short_pool * (combined[t] / tot)
                    sz = dollar_per_name
                    if sz <= 0:
                        continue
                    en_spread = spread_v[date_i, ti]
    ...
    max_share = max((x[2] for x in max_single_position_share), default=0.0)
    return pv, tl, trl, max_share
...
def main():
    ...
    for level in [100_000, 1_000_000, 10_000_000]:
        ...
        pv, tl, trl, max_share = run_backtest(close, close_adj, spread_df, volume, filing_index,
        ...
        m = {"capital_level": level, "cagr": cagr(main_pv), "sharpe": sharpe(main_pv),
             "max_drawdown": max_drawdown(main_pv), "n_trims": len(trl), "max_single_position_share": max_share}
        ...
        print(f"    Sharpe(ex-2020)={sharpe_ex2020:.4f}  Kurtosis={kurt:.1f}  Storsta enskilda positionsandel={max_share:.2%}")
    ...
    cond5 = capacity_ratio > HYP079_CAPACITY_RATIO
    ...
    print(f"  Villkor 5 (kapacitetskvot > {HYP079_CAPACITY_RATIO:.4f}): {capacity_ratio:.4f} -> {'PASS' if cond5 else 'FAIL'}")
    print(f"  => {overall}\n")
