"""
HYP-075: Positionstak pa HYP-072 - symmetrisk trimning av positioner som
mer an tredubblats. Se
research/hypothesis_registry/HYP-075-positionstak-hyp072.yaml for det
lasta kriteriet.

Identisk signal/klassificering och manatlig ombalansering som HYP-072:s
deltest A (full_ls). NY REGEL: varje handelsdag, om en oppen positions
varde (long) eller skuld (kort) ar >= 3,0x dess varde vid senaste
entry/trimning, trimma tillbaka till det referensvardet NASTA
handelsdag (en dags exekveringsfordrojning, som resten av registrets
handelsedetekterade regler).
"""
...
# ════════════════════════════════════════════════════════════
#  PARAMETRAR (lasta i HYP-075:s pass_fail_criterion)
# ════════════════════════════════════════════════════════════
...
# HYP-072:s egna redan uppmatta jamforelsetal ($100k, deltest A), fran
# dess registerpost - anvands har som lasta jamforelsevarden, INTE
# omraknade eller gissade.
...
# ════════════════════════════════════════════════════════════
#  MOTOR (HYP-072:s full_ls-lage + daglig positionstakskontroll)
# ════════════════════════════════════════════════════════════
def run_backtest(close, close_adj, spread_df, volume, filing_index, universe_by_month, capital_level: float):
    ...
    for date_i in range(start_idx, len(close.index)):
        ...
        # STEG 1: exekvera trimningar upptackta igar (en dags fordrojning).
        ...
        # STEG 3: daglig vardering + upptackt av NYA trimningshandelser (for imorgon).
...
def main():
    ...
    for level in [100_000, 1_000_000, 10_000_000]:
        ...
        pv, tl, trl = run_backtest(close, close_adj, spread_df, volume, filing_index, universe_by_month, level)
