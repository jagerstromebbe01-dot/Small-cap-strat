"""
HYP-077: Kontinuerlig shelf-intensitet med decilrangordning - alternativ
konstruktion till HYP-072. Se
research/hypothesis_registry/HYP-077-kontinuerlig-shelf-intensitet-decil.yaml
for det lasta kriteriet.

Ersatter HYP-072:s binara "aktiv"/"ren"-klassificering med en
kontinuerlig, tidsviktad shelf_score (90-dagars halveringstid, 24
manaders fonster) och decilrangordning: SHORT toppdecilen, LONG
bottendecilen. Tie-break for lika score: tickersymbol alfabetiskt.
"""
...
# ════════════════════════════════════════════════════════════
#  PARAMETRAR (lasta i HYP-077:s pass_fail_criterion)
# ════════════════════════════════════════════════════════════
...
# ════════════════════════════════════════════════════════════
#  KONTINUERLIG SHELF-INTENSITET (ny signal, HYP-077)
# ════════════════════════════════════════════════════════════
DECAY_LAMBDA = math.log(2) / DECAY_HALFLIFE_DAYS


def shelf_score_asof(filings, as_of_ts):
    score = 0.0
    for f in filings:
        ...
        if fts >= as_of_ts:
            continue
        age_days = (as_of_ts - fts).days
        if age_days > SCORE_LOOKBACK_DAYS:
            continue
...
# ════════════════════════════════════════════════════════════
#  MOTOR (samma engine-struktur som HYP-072:s full_ls, decilbaserad signal)
# ════════════════════════════════════════════════════════════
        ...
        if date in rebal_set:
            ...
            for t in eligible:
                ...
                if np.isnan(cp) or cp <= 0:
                    continue
                sc = shelf_score_asof(filing_index.get(t, []), as_of)
                scored.append((t, sc))

            n_decile = max(1, round(len(scored) * DECILE_FRACTION)) if scored else 0
            scored_desc = sorted(scored, key=lambda x: (-x[1], x[0]))
            scored_asc = sorted(scored, key=lambda x: (x[1], x[0]))
            target_short = {t for t, _ in scored_desc[:n_decile]}
            target_long = {t for t, _ in scored_asc[:n_decile]}

            n_zero_in_long = sum(1 for t in target_long if dict(scored)[t] == 0.0)
            monthly_disclosure.append({"date": str(date.date()), "n_eligible": len(scored),
                                        "n_decile": n_decile, "n_zero_in_long_decile": n_zero_in_long})
...
def main():
    ...
    for level in [100_000, 1_000_000, 10_000_000]:
        ...
        pv, tl, disclosure = run_backtest(close, close_adj, spread_df, volume, filing_index, universe_by_month, level)
