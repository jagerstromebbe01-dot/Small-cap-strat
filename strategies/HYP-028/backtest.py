...
HYP-028 (BATCH-001, kandidat 4/5): TREDJE lager, ett per-position
stop-loss pa HYP-018:s ORIGINALTROSKEL -20%, EFTER bade SPY-overlayen
och sektor-triggern - MEN med krav pa 3 PAFOLJANDE handelsdagar under
trosklen innan den utloser (i stallet for att andra trosklens DJUP som
HYP-025/026/027/029). Se
research/hypothesis_registry/HYP-028-stoploss-20pct-3dagar.yaml for det
lasta kriteriet. Malfall: HYP-018 (samma troskel pa HYP-017, enda-dags-
kontroll, FAILED - for manga falska larm fran normala tillfalliga
nedgangar).
"""
...
# Per-position stop-loss (ny i HYP-028, BATCH-001 kandidat 4/5 - se
# pass_fail_criterion, lockat, vald INNAN nagon backtest i batchen kordes.
# SAMMA troskel som HYP-018 (-20%), men kraver 3 pafoljande dagar under
# trosklen - testar OM dimensionen an trosklens djup)
STOP_LOSS_THRESHOLD = -0.20
STOP_LOSS_CONFIRM_DAYS = 3
...
def run_backtest(close, close_adj, hedge, vol_df, beta_df, spread_df, volume,
    ...
    for date_i in range(start_idx, len(close.index)):
        ...
        # ── Per-position stop-loss (ny, HYP-028/BATCH-001): DAGLIG
        # kontroll, EFTER bade SPY-overlayen och sektor-triggern. Kraver
        # STOP_LOSS_CONFIRM_DAYS (=3 har) pafoljande dagar under trosklen
        # innan den utloser - rakningen nollstalls om priset atergar over
        # trosklen nagon dag under fonstret.
        for t in list(holdings.keys()):
            ti = tidx[t]
            cp = prices_v[date_i, ti]
            if np.isnan(cp):
