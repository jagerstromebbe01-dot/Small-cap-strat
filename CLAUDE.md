...
4. **Paper trading is blocked on live data coverage, not code (found 2026-07-31).** Two independent attempts to build a "sharp" (forward, no real money) paper-trading feed for HYP-023 both hit the same wall: this EODHD API key's real-time daily coverage for small/micro-cap US tickers lags by weeks to months for most names, even though SPY and a couple of others are fully current. First attempt (frozen Dec-2024 universe list, `data/update_ohlcv_current.py`) found 184/222 tickers stale >120 days. Second attempt (`data/rebuild_current_universe.py`, properly rebuilding today's $100M-$2B band from fresh SEC EDGAR shares outstanding × fresh price, not just reusing the old list) found only 11/383 reasonable candidates with a price inside 60 days, and only one (OPI) actually current as of yesterday. This rules out "bad universe-membership logic" as the cause — it's a data-vendor coverage gap. Needs either a better-tier/different data source for live small-cap prices, or an explicit decision to run paper trading on degraded (weeks-to-months-stale) marks. `paper_trading/HYP-023/compute_current_state.py` exists and works once real current data is available for enough of the universe to rank a meaningful decile.
